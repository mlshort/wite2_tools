"""
Module for replacing specific Ground Element IDs across WiTE2 _unit CSV files.
This module uses the base `process_csv_in_place` wrapper to safely perform
atomic file replacements.

This module provides functionality to parse a War in the East 2 (WiTE2) unit CSV
file and globally replace all instances of a specific Ground Element ID with a
new ID. It scans through all 32 unit squad slots (`sqd.u0` through `sqd.u31`)
and performs an integer-based comparison to ensure accurate replacements.
...
Command Line Usage:
    python replace_unit_ground_element.py [-h] old_ge_id new_ge_id

Arguments:
    old_ge_id (int): The existing Ground Element ID to be replaced.
    new_ge_id (int): The new Ground Element ID to insert.

Example:
    $ python replace_unit_ground_element.py 105 110
...
def replace_unit_ground_element(unit_file_path: str, old_ge_id: int, new_ge_id: int) -> int:
    """
    Replaces a specific Ground Element ID with a new one using integer-based comparison.
    """
    log.info("Task Start: Replace GE ID %d with %d in '%s'",
             old_ge_id, new_ge_id, os.path.basename(unit_file_path))

    # Define the specific logic for processing a Unit row
    def process_row(row: dict, _row_idx:int) -> tuple[dict, bool]:
        was_modified = False
