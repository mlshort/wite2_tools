...
file and globally replace all instances of a specific Ground Element WID with a
...
def replace_unit_ground_element(unit_file_path: str, old_ge_id: int, new_ge_id: int) -> int:
    """
    Replaces a specific Ground Element WID with a new one using integer-based comparison.
    """
    ...
    log.info("Task Complete: Modified %d rows containing the old WID %d.", total_updates, old_ge_id)
