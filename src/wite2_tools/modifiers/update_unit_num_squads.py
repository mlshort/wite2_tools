...
Command Line Usage:
    python -m wite2_tools.cli update-num [-h] ob_id wid old_num_squads new_num_squads

Arguments:
    ob_id (int): Target unit's TOE(OB) ID (Order of Battle ID).
    wid (int): Unit's Element WID containing the squads to change.
...
def update_unit_num_squads(unit_file_path: str, target_ob_id: int, wid: int, old_num_squads: int, new_num_squads: int) -> int:
    """
