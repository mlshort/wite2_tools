...
--------------------
* Chronological Bounds: Ensures an OB does not expire (Last Year/Month)
...
* Upgrade Path Integrity: Detects upgrade dead-ends (pointing to a non-existent
  OB ID) and infinite self-upgrade loops.
* Slot Consistency: Flags negative squad quantities and prevents intra-template
  duplication (assigning the same WID to multiple slots in the same OB).
...
def _check_squad_slots(ob_id: int,
    ...
    for i in range(MAX_SQUAD_SLOTS):
        ...
        if qty != 0 and sqd_id == 0:
            log.warning("OB (ID %d): Ghost Squad! %s has quantity %d but %s "
                        "is '0'",
...
def audit_ob_csv(ob_file_path: str, ground_file_path: str) -> int:
    ...
    try:
        ...
        # 1. PRE-PASS: Collect all valid OB IDs for upgrade checking
        ...
        for _, row in ob_gen:
            ...
            ob_name = parse_str(row.get('name'), "Unknown")
