...
def test_real_data_paths_exist():
    ...
    """
    assert os.path.exists(CONF_UNIT_FULL_PATH), \
        f"Missing: {CONF_UNIT_FULL_PATH}"
    assert os.path.exists(CONF_OB_FULL_PATH), \
        f"Missing: {CONF_OB_FULL_PATH}"
    assert os.path.exists(CONF_GROUND_FULL_PATH), \
        f"Missing: {CONF_GROUND_FULL_PATH}"
...
def test_load_global_inventory_real_data():
    """
    Stress test: Runs the inventory counter against the configured
    game data files.
    """
    inventory = count_global_unit_inventory(CONF_UNIT_FULL_PATH,
                                            CONF_GROUND_FULL_PATH)
