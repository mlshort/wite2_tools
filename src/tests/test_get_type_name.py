...
def clear_caches():
    ...
    This ensures that Test A doesn't accidentally use the cached data from
    Test B!
    """
...
def test_get_ob_full_name_success(mock_ob_csv):
    """
    Verifies that the TOE(OB) lookup correctly parses and combines
    strings.
    """
...
def test_get_ob_full_name_missing_id(mock_ob_csv):
    ...
    assert get_ob_full_name(mock_ob_csv, 0) == "Unk (0)"
    # 0 was skipped during parse
